...
import ooadj.chessmp.domain.piece.Path;
import ooadj.chessmp.domain.piece.Piece;
import ooadj.chessmp.domain.piece.PieceFactory;
import ooadj.chessmp.domain.piece.Position;
import ooadj.chessmp.domain.piece.Symbol;
...
public class Board {
    ...
    private void validateColor(Piece source, Piece target) {
        if (source.isSameColor(target)) {
            throw new NotMovableException("Invalid Move. Same color at target position.");
        }
    }
    ...
    private void validateBlank(Position position) {
        ...
        if (!piece.isBlank()) {
            throw new NotMovableException("Square is not blank.");
        }
    }
